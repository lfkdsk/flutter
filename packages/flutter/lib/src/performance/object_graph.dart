...
class MergedObjectVertex {
  ...
  bool operator ==(dynamic other) => _id == other._id && _graph == other._graph;
  int get hashCode => _id;
...
class _SnapshotGraph implements SnapshotGraph {
  ...
  String _describeObject(int oid) {
    ...
    dynamic data = _nonReferenceData[oid];
    if (data == null) {
      return cls;
    } else {
      return "$cls($data)";
  ...
  void _readObjects(_ReadStream stream) {
    ...
    var nonReferenceData = new List<dynamic>(N + 1);
  ...
  void _buildDominators() {
    ...
    var buckets = new List<dynamic>(N + 1);
  ...
  void _mergeDominatorSiblings() {
    ...
    mergeChildrenAndSort(dynamic parent1, dynamic end) {
