...
class FlutterVersion {
  ...
  FlutterVersion([this._clock = const SystemClock(), this._workingDirectory]) {
    ...
    // BD ADD:
    _bdFrameworkVersion =
        BDGitTagVersion.determine(processUtils, workingDirectory: _workingDirectory, fetchTags: false).frameworkVersionFor(_frameworkRevision);
  }
  ...
  static GitTagVersion determine(ProcessUtils processUtils, {String workingDirectory, bool fetchTags = false}) {
    ...
    return parse(
      _runGit(
        ...
        'git describe --match *.*.* --exclude bd*.*.* --first-parent --long --tags',
        processUtils,
        workingDirectory,
      )
    );
...
class BDGitTagVersion {
  ...
  static BDGitTagVersion determine(ProcessUtils processUtils, {String workingDirectory, bool fetchTags = false}) {
    return parse(
        _runGit('git describe --match bd*.*.* --first-parent --long --tags', processUtils, workingDirectory));
  }
