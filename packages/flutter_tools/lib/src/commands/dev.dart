...
import 'package:flutter_tools/src/device.dart' as import_device;
...
class DevAndroidCommand extends FlutterCommand {
  ...
  Future<void> validateCommand() async {
    ...
    final import_device.Device device = await findTargetDevice();
    if (device == null) {
    ...
    _apkDir = projectRoot.childDirectory('apks');
  }
...
class DevIOSCommand extends FlutterCommand {
  ...
  List<import_device.Device> _devices;
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final BuildableIOSApp iosProject = await applicationPackages
            .getPackageForPlatform(TargetPlatform.ios, buildInfo)
        as BuildableIOSApp;
    ...
    print('Building framework for $iosProject in ${getNameForBuildMode(buildInfo.mode)} mode...');
    ...
    await _produceFlutterFramework(outputDirectory, buildInfo, iPhoneBuildOutput, simulatorBuildOutput, modeDirectory);
  ...
  Future<void> _produceFlutterFramework(Directory outputDirectory, BuildInfo buildInfo, Directory iPhoneBuildOutput, Directory simulatorBuildOutput, Directory modeDirectory) async {
    final Status status = logger.startProgress(' ├─Populating Flutter.framework...', timeout: timeoutConfiguration.fastOperation);
    final String engineCacheFlutterFrameworkDirectory = artifacts.getArtifactPath(Artifact.flutterFramework, platform: TargetPlatform.ios, mode: buildInfo.mode);
    ...
    fsUtils.copyDirectorySync(fs.directory(engineCacheFlutterFrameworkDirectory), fatFlutterFrameworkCopy);
    ...
    if (boolArg('xcframework')) {
      ...
      fsUtils.copyDirectorySync(fatFlutterFrameworkCopy, armFlutterFrameworkDirectory);
      ...
      fsUtils.copyDirectorySync(fatFlutterFrameworkCopy, simulatorFlutterFrameworkDirectory);
  ...
  Future<void> _produceAppFramework(BuildInfo buildInfo, Directory iPhoneBuildOutput, Directory simulatorBuildOutput, Directory modeDirectory) async {
    ...
    if (buildInfo.mode == BuildMode.debug) {
      ...
      await _produceAotAppFrameworkIfNeeded(buildInfo, iPhoneBuildOutput, destinationAppFrameworkDirectory);
    }
  ...
  Future<void> _produceAotAppFrameworkIfNeeded(BuildInfo buildInfo, Directory iPhoneBuildOutput, Directory destinationAppFrameworkDirectory) async {
    if (buildInfo.mode == BuildMode.debug) {
      return;
    }
    ...
    const String appFrameworkName = 'App.framework';
    fsUtils.copyDirectorySync(iPhoneBuildOutput.childDirectory(appFrameworkName), destinationAppFrameworkDirectory);
    status.stop();
  }
